...
const serializers = {
  ...
  const emoji = item.payload.action === 'created' ? '✔️' : (item.payload.action === 'edited' ? '' : '❌')
    return `${emoji} ${capitalize(item.payload.action)} PR #${item.payload.pull_request.number} in ${
      item.repo.name
  },
